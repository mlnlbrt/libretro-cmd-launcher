...
void retro_get_system_info(struct retro_system_info *info)
{
   ...
   info->library_name     = "Command Launcher";
#ifndef GIT_VERSION
#define GIT_VERSION ""
#endif
   info->library_version  = "1.0.0" GIT_VERSION;
   info->need_fullpath    = true;
   info->valid_extensions = "sh|bash";
}
 ...
 *
 * Once the core has run, we will attempt to exit, since Command is done.
 */
void retro_run(void)
{
   // Clear the display.
   ...
   // Shutdown the environment now that Command has loaded and quit.
...
bool retro_load_game(const struct retro_game_info *info)
{
   ...
   if (system(command) == 0) {
      printf("libretro-cmd-launcher: Completed\n");
      return true;
   }

   printf("libretro-cmd-launcher: Failed running Command. Install it and try again.\n");
   return false;
}
